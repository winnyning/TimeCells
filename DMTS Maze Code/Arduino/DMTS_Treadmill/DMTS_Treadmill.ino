...
int servopos1[]   = {600, 250}; // study spinner
...
int studystrobe[] = {41, 42}; // study beads or study yarn
int cstrobe[]     = {44, 44, 45, 45};
int icstrobe[]    = {45, 45, 44, 44};  
...
int beamstat      = 0;
int serialdata    = 0;
int ratrunning   = 0; // has he broken the tmill beam
...
void loop ()
{
  ...
  else if (taskphase == 2)
  {
    ...
    {
      // one ms square pulse
      digitalWrite(studystrobe[trialobj[serialdata]], 1);
      delay(2);
      digitalWrite(studystrobe[trialobj[serialdata]], 0);
      holdtime++;
    }
    else
    {
    ...
    if (holdtime > BEAMTIME)
    {
      ...
      // advance task phase
      taskphase = 3;
      // say that the rat hasnt gotten on the treadmill
      ratrunning = 0;
  ...
  else if (taskphase == 4)
  {
    ...
    if (!digitalRead(testcpins[trialtype]))
    {
      feedir++;
      digitalWrite(cstrobe[trialtype], 1);
      delay(2);
      digitalWrite(cstrobe[trialtype], 0);
    }
    if (!digitalRead(testicpins[trialtype]))
    {
      beepir++;
      digitalWrite(icstrobe[trialtype], 1);
      delay(2);
      digitalWrite(icstrobe[trialtype], 0);
    }
